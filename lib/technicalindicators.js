    ...
    this.generator = (function* () {
      ...
      while (true) {
        ...
        if (calcSMA) {
          ...
          let pw = format((200 * (upper - lower)) / (upper + lower))
          result = {
            middle: middle,
            upper: upper,
            lower: lower,
...
class MorningStar extends CandlestickFinder {
  ...
  logic(data) {
    ...
    let isSmallBodyExists =
      firstdaysLow > seconddaysLow && firstdaysLow > seconddaysHigh
...
class BullishInvertedHammerStick extends CandlestickFinder {
  ...
  logic(data) {
    ...
    isBullishInvertedHammer =
      isBullishInvertedHammer &&
      daysClose - daysOpen <= 2 * (daysHigh - daysClose)
...
class BearishHammerStick extends CandlestickFinder {
  ...
  logic(data) {
    ...
    isBearishHammer =
      isBearishHammer && daysOpen - daysClose <= 2 * (daysClose - daysLow)
...
class IchimokuCloud extends Indicator {
  constructor(input) {
    ...
    var currentConversionData = new FixedSizeLinkedList(
      params.conversionPeriod * 2,
      true,
      true,
      false
    )
    var currentBaseData = new FixedSizeLinkedList(
      params.basePeriod * 2,
      true,
      true,
      false
    )
    var currenSpanData = new FixedSizeLinkedList(
      params.spanPeriod * 2,
      true,
      true,
      false
...
class ChandelierExit extends Indicator {
  constructor(input) {
    ...
    var dataCollector = new FixedSizeLinkedList(
      input.period * 2,
      true,
      true,
      false
    ...
    this.generator = (function* () {
      ...
      while (true) {
        ...
        if (
          dataCollector.totalPushed >= 2 * input.period &&
          atr$$1 != undefined
        ) {
          result = {
