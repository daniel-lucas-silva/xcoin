...
module.exports = function keyboard(s, conf, core) {
  ...
  function listKeys() {
    ...
    keyMap.set(
      'O',
      'show current symbol options in a dirty view (full list)'.cyan
    )
    ...
    for (let ii = 1; ii < 10; ii++) {
      keyMap.set(
        'Alt+' + ii,
        `select picker[${ii + 10}] or symbol[${ii + 10}] to`.cyan
      )
    }
    /* for (let ii = 1; ii < 10; ii++) {
      ...
      printLog(
        [
          ...
          z(
            20,
            '         ' +
              n(s.symbols[symbol.product_id].usdtProfit).format('0.00')[
                s.symbols[symbol.product_id].usdtProfit > 0 ? 'green' : 'red'
              ] +
              '    ',
            ' '
          ),
          z(
            20,
            '         ' +
              (n(100 * s.symbols[symbol.product_id].profit).format('0.00') +
                '%')[
                s.symbols[symbol.product_id].usdtProfit > 0 ? 'green' : 'red'
              ],
            ' '
          ...
          z(
            20,
            '      ' +
              (n(100 * s.symbols[symbol.product_id].dynamicProfit).format(
                '0.00'
              ) + '%')[
                s.symbols[symbol.product_id].dynamicProfit > 0 ? 'green' : 'red'
              ],
            ' '
          )
        ].join('')
  ...
  function executeKey(key, info, symbol, cb) {
    ...
    if (!symbol)
      symbol = s.options.symbols[keyIndex - 1] || s.options.symbols[0]
    printLog(
      'Execute key ' +
        (key ? key.green : '') +
        ' symbol ' +
        (symbol ? symbol.product_id.green : '')
    )
    if (key === 'l') {
      listKeys()
      if (cb) cb()
      ...
      printLog(
        'manual'.cyan + ' BUY SHORT'.green + ' command executed'.cyan,
        true
      )
      ...
      printLog(
        'manual'.cyan + ' SELL SHORT'.red + ' command executed'.cyan,
        true
      )
      ...
      printLog(
        'Select symbol index '.cyan +
          (' ' + keyIndex).green +
          ' symbol ' +
          s.options.symbols[keyIndex - 1].product_id.green,
        true
      )
    } else if (key === 'S' && !info.ctrl) {
      let boughtSymbols = s.options.symbols.filter((ss) => {
        ...
        if (
          s.symbols[ss.product_id] &&
          s.symbols[ss.product_id].action &&
          s.symbols[ss.product_id].action === 'bought'
        ) {
          return true
        }
        return false
