...
module.exports = function panacea(s, conf, engine, keyboard) {
  ...
  var wsServer = {
    ...
    run: function () {
      if (
        !s.options.output.panacea.port ||
        s.options.output.panacea.port === 0
      ) {
      ...
      this.wss.on('connection', (ws) => {
        ...
        debug.msg(
          'Websocket new client join '.cyan +
            this.wss.clients.size.toString().cyan
        )
        this.init(ws)
        ws.on('pong', () => {
          ws.isAlive = true
        ...
        ws.on('message', (data) => {
          try {
            ...
            /** client user auth */
            if (
              message.action !== 'getTickers' &&
              message.action !== 'checkVersion' &&
              message.action !== 'getProducts' &&
              message.action !== 'getPickerNormal'
            ) {
              if (
                !message.apiKey ||
                message.apiKey !== conf.secret.keys.apiKey
              ) {
                self.reply(ws, {
                  action: 'none',
