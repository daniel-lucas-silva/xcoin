...
const addSymbol = (message, cb, s, conf, engine) => {
  let shouldAddSymbols = message.symbols.filter((m) =>
    s.options.symbols.every((p) => p.normalized !== m)
  )
  ...
  let boughtSymbols = s.options.symbols.filter((ss) => {
    ...
    if (
      ...
      (s.symbols[ss.product_id].action === 'bought' ||
        s.symbols[ss.product_id].action === 'partSell')
    ) {
      return true
    }
    return false
